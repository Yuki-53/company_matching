...
def data_test_split(data):
    ...
    database = database[:10000]  # limit db only for demo
...
def prep_data(data, company_name):
    ...
    data['len_intersection'] = data.apply(
        ...
        axis = 1)
    data['fuzz_ratio'] = data.apply(
        lambda x: fuzz.ratio(x['name_1'], x['name_2']), 
        axis = 1)
    data['jaccard'] = data.apply(
        lambda x: x['len_intersection']/(x['len_str_1'] + x['len_str_2'] - x['len_intersection']),
        axis = 1)
    return data
...
def get_metrics(preds, true, labels, ks = [5, 10, 20, 50]):
    ...
    for k in ks:
        print(
            f'top {k} accuracy score:', 
            top_k_accuracy_score(true, preds, k=k, labels=labels))
